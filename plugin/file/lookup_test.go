...
func BenchmarkFileLookup(b *testing.B) {
	...
	ctx := context.TODO()
	rec := dnstest.NewRecorder(&test.ResponseWriter{})
	tc := test.Case{Qname: "www.miek.nl.", Qtype: dns.TypeA, Answer: []dns.RR{test.CNAME("www.miek.nl.	1800	IN	CNAME	a.miek.nl."), test.A("a.miek.nl.	1800	IN	A	139.162.196.78")}}
	m := tc.Msg()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		fm.ServeDNS(ctx, rec, m)
