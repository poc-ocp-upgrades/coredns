...
func TestSetup(t *testing.T) {
	...
	tests := []struct {
		...
		expectedErr	string
	}{{"forward . 127.0.0.1", false, ".", nil, 2, options{}, ""}, {"forward . 127.0.0.1 {\nexcept miek.nl\n}\n", false, ".", nil, 2, options{}, ""}, {"forward . 127.0.0.1 {\nmax_fails 3\n}\n", false, ".", nil, 3, options{}, ""}, {"forward . 127.0.0.1 {\nforce_tcp\n}\n", false, ".", nil, 2, options{forceTCP: true}, ""}, {"forward . 127.0.0.1 {\nprefer_udp\n}\n", false, ".", nil, 2, options{preferUDP: true}, ""}, {"forward . 127.0.0.1 {\nforce_tcp\nprefer_udp\n}\n", false, ".", nil, 2, options{preferUDP: true, forceTCP: true}, ""}, {"forward . 127.0.0.1:53", false, ".", nil, 2, options{}, ""}, {"forward . 127.0.0.1:8080", false, ".", nil, 2, options{}, ""}, {"forward . [::1]:53", false, ".", nil, 2, options{}, ""}, {"forward . [2003::1]:53", false, ".", nil, 2, options{}, ""}, {"forward . a27.0.0.1", true, "", nil, 0, options{}, "not an IP"}, {"forward . 127.0.0.1 {\nblaatl\n}\n", true, "", nil, 0, options{}, "unknown property"}, {`forward . ::1
...
func TestSetupTLS(t *testing.T) {
	...
	tests := []struct {
		...
		expectedErr		string
				...
				tls_servername dns
			}`, false, "dns", ""}, {`forward . 127.0.0.1 {
				tls_servername dns
			}`, false, "", ""}, {`forward . 127.0.0.1 {
				tls
			}`, false, "", ""}, {`forward . tls://127.0.0.1`, false, "", ""}}
	for i, test := range tests {
		c := caddy.NewTestController("dns", test.input)
		f, err := parseForward(c)
		if test.shouldErr && err == nil {
...
func TestSetupResolvconf(t *testing.T) {
	...
	if err := ioutil.WriteFile(resolv, []byte(`nameserver 10.10.255.252
