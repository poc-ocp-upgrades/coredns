...
func TestAutoParse(t *testing.T) {
	...
	tests := []struct {
		...
		expectedTo		[]string
			...
			}`, false, "/tmp", "${1}", `db\.(.*)`, []string{"127.0.0.1:53"}}, {`auto 10.0.0.0/24 {
				directory /tmp
			}`, false, "/tmp", "${1}", `db\.(.*)`, nil}, {`auto {
				directory /tmp
				no_reload
			}`, false, "/tmp", "${1}", `db\.(.*)`, nil}, {`auto {
				directory /tmp (.*) bliep
			}`, false, "/tmp", "bliep", `(.*)`, nil}, {`auto {
				...
				upstream 8.8.8.8
			}`, false, "/tmp", "bliep", `(.*)`, []string{"127.0.0.1:53", "127.0.0.2:53"}}, {`auto example.org {
				directory
			}`, true, "", "${1}", `db\.(.*)`, nil}, {`auto example.org {
				directory /tmp * {1}
			}`, true, "", "${1}", ``, nil}, {`auto example.org {
				directory /tmp * {1} aa
			}`, true, "", "${1}", ``, nil}, {`auto example.org {
				directory /tmp .* {1}
			}`, true, "", "${1}", ``, nil}, {`auto example.org {
