...
func TestLookupCNAMEExternal(t *testing.T) {
	...
	zone.Upstream, _ = upstream.New([]string{"8.8.8.8:53"})
	fm := File{Next: test.ErrorHandler(), Zones: Zones{Z: map[string]*Zone{name: zone}, Names: []string{name}}}
	ctx := context.TODO()
...
var exernalTestCases = []test.Case{{Qname: "external.example.org.", Qtype: dns.TypeA, Answer: []dns.RR{test.CNAME("external.example.org. 1800	CNAME	www.example.net."), test.A("www.example.net.	303	IN	A	93.184.216.34")}}}
