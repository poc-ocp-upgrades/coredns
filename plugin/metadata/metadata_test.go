...
func TestMetadataServeDNS(t *testing.T) {
	...
	expectedMetadata := []testProvider{{"test/key1": func() string {
		return "testvalue1"
	}}, {"test/key2": func() string {
		return "two"
	}, "test/key3": func() string {
		return "testvalue3"
	}}}
	providers := []Provider{}
	...
	next := &testHandler{}
	m := Metadata{Zones: []string{"."}, Providers: providers, Next: next}
	ctx := context.TODO()
...
func TestLabelFormat(t *testing.T) {
	...
	labels := []struct {
		label	string
		isValid	bool
	}{{"plugin/LABEL", true}, {"p/LABEL", true}, {"plugin/L", true}, {"LABEL", false}, {"plugin.LABEL", false}, {"/NO-PLUGIN-NOT-ACCEPTED", false}, {"ONLY-PLUGIN-NOT-ACCEPTED/", false}, {"PLUGIN/LABEL/SUB-LABEL", false}, {"/", false}, {"//", false}}
