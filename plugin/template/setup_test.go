...
func TestSetupParse(t *testing.T) {
	...
	tests := []struct {
		...
		shouldErr	bool
	}{{`template`, true}, {`template X`, true}, {`template ANY`, true}, {`template ANY X`, true}, {`template ANY ANY (?P<x>`, true}, {`template ANY ANY {

			}`, true}, {`template ANY ANY .* {
				notavailable
			}`, true}, {`template ANY ANY {
				answer
			}`, true}, {`template ANY ANY {
			...
			template IN MX example. {
				...
				}`, false}, {`template ANY ANY up.stream.local {
