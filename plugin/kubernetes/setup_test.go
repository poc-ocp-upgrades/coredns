...
func TestKubernetesParse(t *testing.T) {
	...
	tests := []struct {
		...
		expectedNSCount		int
		expectedResyncPeriod	time.Duration
		expectedLabelSelector	string
		expectedPodMode		string
		expectedFallthrough	fall.F
		expectedUpstreams	[]string
	}{{`kubernetes coredns.local`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local test.local`, false, "", 2, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	endpoint http://localhost:9090 http://localhost:9091
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	namespaces demo
}`, false, "", 1, 1, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	namespaces demo test
}`, false, "", 1, 2, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    resyncperiod 30s
}`, false, "", 1, 0, 30 * time.Second, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    resyncperiod 15m
}`, false, "", 1, 0, 15 * time.Minute, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    labels environment=prod
}`, false, "", 1, 0, defaultResyncPeriod, "environment=prod", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    labels environment in (production, staging, qa),application=nginx
}`, false, "", 1, 0, defaultResyncPeriod, "application=nginx,environment in (production,qa,staging)", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local test.local {
    resyncperiod 15m
	endpoint http://localhost:8080
	namespaces demo test
    labels environment in (production, staging, qa),application=nginx
    fallthrough
}`, false, "", 2, 2, 15 * time.Minute, "application=nginx,environment in (production,qa,staging)", podModeDisabled, fall.Root, nil}, {`kubernetes coredns.local {
    endpoint
}`, true, "rong argument count or unexpected line ending", -1, -1, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	namespaces
}`, true, "rong argument count or unexpected line ending", -1, -1, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    resyncperiod
}`, true, "rong argument count or unexpected line ending", -1, 0, 0 * time.Minute, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    resyncperiod 15
}`, true, "unable to parse resync duration value", -1, 0, 0 * time.Second, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    resyncperiod abc
}`, true, "unable to parse resync duration value", -1, 0, 0 * time.Second, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    labels
}`, true, "rong argument count or unexpected line ending", -1, 0, 0 * time.Second, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    labels environment in (production, qa
}`, true, "unable to parse label selector", -1, 0, 0 * time.Second, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	pods disabled
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	pods insecure
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeInsecure, fall.Zero, nil}, {`kubernetes coredns.local {
	pods verified
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeVerified, fall.Zero, nil}, {`kubernetes coredns.local {
	pods giant_seed
}`, true, "rong value for pods", -1, 0, defaultResyncPeriod, "", podModeVerified, fall.Zero, nil}, {`kubernetes coredns.local {
	fallthrough ip6.arpa inaddr.arpa foo.com
}`, false, "rong argument count", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.F{Zones: []string{"ip6.arpa.", "inaddr.arpa.", "foo.com."}}, nil}, {`kubernetes coredns.local {
	upstream 13.14.15.16:53
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, []string{"13.14.15.16:53"}}, {`kubernetes coredns.local {
	upstream 13.14.15.16orange
}`, true, "not an IP address or file: \"13.14.15.16orange\"", -1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local
kubernetes cluster.local`, true, "this plugin", -1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	kubeconfig
}`, true, "Wrong argument count or unexpected line ending after", -1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	kubeconfig file context extraarg
}`, true, "Wrong argument count or unexpected line ending after", -1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
	kubeconfig file context
}`, false, "", 1, 0, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}, {`kubernetes coredns.local {
    endpoint http://localhost:9090 https://localhost:9091
}`, true, "multiple endpoints can only accept http", -1, -1, defaultResyncPeriod, "", podModeDisabled, fall.Zero, nil}}
...
func TestKubernetesParseEndpointPodNames(t *testing.T) {
	...
	endpoint_pod_names
}`, false, "", true}, {`kubernetes coredns.local {
	endpoint_pod_names giant_seed
}`, true, "rong argument count or unexpected", false}, {`kubernetes coredns.local {
}`, false, "", false}}
	for i, test := range tests {
		c := caddy.NewTestController("dns", test.input)
		k8sController, err := kubernetesParse(c)
...
func TestKubernetesParseNoEndpoints(t *testing.T) {
	...
	noendpoints
}`, false, "", false}, {`kubernetes coredns.local {
	noendpoints ixnay on the endpointsay
}`, true, "rong argument count or unexpected", true}, {`kubernetes coredns.local {
}`, false, "", true}}
	for i, test := range tests {
		c := caddy.NewTestController("dns", test.input)
		k8sController, err := kubernetesParse(c)
...
}`, true, "unable to parse ignore value", false}, {`kubernetes coredns.local {
