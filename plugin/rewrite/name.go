...
func newNameRule(nextAction string, args ...string) (Rule, error) {
	...
	if len(args) < 7 {
		...
		case ExactMatch:
			...
			return &exactNameRule{nextAction, rewriteQuestionFrom, rewriteQuestionTo, ResponseRule{Active: true, Type: "name", Pattern: rewriteAnswerFromPattern, Replacement: rewriteQuestionFrom}}, nil
		case PrefixMatch:
			return &prefixNameRule{nextAction, rewriteQuestionFrom, rewriteQuestionTo}, nil
		case SuffixMatch:
			return &suffixNameRule{nextAction, rewriteQuestionFrom, rewriteQuestionTo}, nil
		case SubstringMatch:
			return &substringNameRule{nextAction, rewriteQuestionFrom, rewriteQuestionTo}, nil
		case RegexMatch:
			...
			rewriteQuestionTo := plugin.Name(args[2]).Normalize()
			return &regexNameRule{nextAction, rewriteQuestionFromPattern, rewriteQuestionTo, ResponseRule{Type: "name"}}, nil
		default:
	...
	if len(args) == 7 {
		if matchType == RegexMatch {
			...
			return &regexNameRule{nextAction, rewriteQuestionFromPattern, rewriteQuestionTo, ResponseRule{Active: true, Type: "name", Pattern: rewriteAnswerFromPattern, Replacement: rewriteAnswerTo}}, nil
		}
